...
2. `go test ./...` — includes `TestPluginSkills`, which enforces that
   `plugin.json.version` equals `PluginVersion` in `cmd/rakko/main.go` and that
...
```bash
gh skill install ideamans/rakkokeyword-cli/plugins/rakkokeyword-cli/skills/rakko-usage --agent copilot
```
