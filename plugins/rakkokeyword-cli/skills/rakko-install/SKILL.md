---
name: rakko-install
description: Make the rakko command available, installing it only if it is missing. Use when another skill reports that `rakko` is not on PATH, or when the user asks to install, update or upgrade the ideamans rakkokeyword CLI. Prefers an already-installed binary, then the latest GitHub release, then a build from source with go install.
license: MIT
compatibility: Requires curl (or wget) and tar to install from a release, or a Go toolchain for the source fallback. Standalone — does not need rakko to be present already. Installs from the public repository github.com/ideamans/rakkokeyword-cli, so no GitHub authentication is needed.
...
Make the `rakko` command usable, doing the least work that achieves it.

## Route 1 — an existing installation on PATH

```bash
command -v rakko && rakko --version && rakko llm | head -1
```
...
Two things invalidate the hit. `rakko` is a short name, so the first line of
`rakko llm` must read `# rakko — reference for AI agents`; if something else
...
curl -fsSL -o /tmp/rakko.tar.gz \
  "https://github.com/ideamans/rakkokeyword-cli/releases/download/${VERSION}/rakko_${VERSION#v}_${OS}_${ARCH}.tar.gz"
```

**The archive is named after the binary, not the repository** — `rakko`, with no
`keyword-cli` suffix: `rakko_<version-without-v>_<os>_<arch>.tar.gz`. Windows
ships a `.zip`. If the download 404s, list the actual assets on the release page
rather than retrying variations.

```bash
tar -xzf /tmp/rakko.tar.gz -C /tmp
mkdir -p ~/.local/bin && mv /tmp/rakko ~/.local/bin/ && chmod +x ~/.local/bin/rakko
```
...
`/cmd/rakko` suffix — installing the module root would not build anything.
...
```bash
rakko --version
rakko metadata languages | head -3   # free, needs no API key: proves it reaches the API
```

Report which route was taken, the version and the install path.

`rakko` cannot do keyword research without an API key from a rakkokeyword
...
rakko auth set-api-key your-key        # or store it in the config file
```

Everything beyond `metadata` spends credits from the user's account — mention
that before the first real query, and see the `rakko-usage` skill for costs.
