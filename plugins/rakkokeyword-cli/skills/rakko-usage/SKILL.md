---
name: rakko-usage
description: Research keywords and SEO with the rakko CLI for the rakkokeyword (ラッコキーワード) API — search-engine suggestions, monthly search volume and SEO difficulty, Google rank checks, competitor and content-gap analysis, and the headings and vocabulary of pages ranking for a keyword. Use when the user asks what people search for, how much volume or difficulty a keyword has, where a site ranks, who its SEO competitors are, or what an article about a topic should cover — especially for the Japanese market.
license: MIT
compatibility: Requires the `rakko` binary on PATH — run the rakko-install skill if it is missing — and a rakkokeyword API key (STANDARD plan or above) in RAKKOKEYWORD_API_KEY. Every call except metadata, histories, status and results spends the user's credits.
allowed-tools: Bash(rakko:*) Bash(jq:*) Bash(command:*) Read Write
---
...
```bash
command -v rakko && rakko auth status
```

Missing binary? Run the `rakko-install` skill. No key? Tell the user to set
...
user. `rakko <command> --help` states the exact cost and `--dry-run` prints the
...
| --- | --- |
| "what do people search around X?" | `rakko suggest-keywords X` |
| "give me every keyword containing X" | `rakko related-keywords X -n 5000` |
| "what do they search or ask next?" | `rakko other-keywords X` (expensive) |
| "which keywords can one article cover?" | `rakko ranking-keywords X` |
| "how much volume / difficulty really?" | `rakko search-volume register --keywords-file kw.txt --wait` |
| "where do we rank for these?" | `rakko search-rank register --keywords-file kw.txt --url https://site/ --wait` |
| "what does competitor X rank for?" | `rakko influx-keywords --target https://x/` |
| "who are our SEO competitors?" | `rakko competitive https://site/` |
| "what should this article contain?" | `rakko headline X` + `rakko co-occurrence X` |

`rakko --help` lists the rest (questions, pages, sites, bulk site research), and
`rakko raw POST /v1/… --data '{…}'` reaches anything unwrapped.

## 4. Read only the reference you need

`rakko llm` is ~1,450 lines — do not read it whole. Take the first chapter for
orientation and pull the others on demand:

```bash
rakko llm | sed -n '1,/^# Metrics/p'                                             # rules, auth, command map
rakko llm --format json | jq -r '.[] | select(.file=="20-schemas.md") | .body'   # response shapes
rakko suggest-keywords --help                                                    # one command's flags and cost
```

## 5. Query with `-f json`, then interpret carefully

```bash
rakko suggest-keywords ラッコ --filter searchVolume.min=100 -n 200 -f json
```
...
`rakko search-volume results <id>`, or find it with `histories`.

## Failure modes

| Symptom | Fix |
| --- | --- |
| `command not found: rakko` | run the `rakko-install` skill |
